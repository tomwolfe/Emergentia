"""
Refactored Engine Module with improved structure and reduced complexity.
This addresses the high cyclomatic complexity in the original Trainer.train_step method.
"""
...
class Trainer:
    ...
    def train_step(self, data_list, dt, epoch=0, max_epochs=2000):
        # Handle sparsity scheduling
        self._handle_sparsity_scheduling(epoch)
        
        # Determine training stage
        is_stage1, is_warmup = self._determine_training_stage(epoch)
        
        # Configure model based on stage
        self._configure_model_for_stage(is_stage1)
        
        # Apply hard revival periodically
        self._apply_hard_revival(epoch)
        
        # Determine if consistency loss should be computed
        compute_consistency = self._should_compute_consistency(epoch, is_stage1, is_warmup)
        ...
        loss_l2, loss_lvr = self._compute_regularization_losses(z_preds, dt)
        loss_smooth = self._compute_latent_smoothing_loss(z_preds)
        
        # Aggregate losses from initial computation
        ...
        loss_hinge = self._compute_hinge_loss(z_preds)
        ...
        for t in range(0, len(data_list), 2):
            batch_t = Batch.from_data_list([data_list[t]]).to(self.device)
            ...
            loss_assign += self._compute_assignment_loss(losses_t, entropy_weight)
            ...
            # Compute alignment and MI losses
            loss_align += self._compute_alignment_loss(z_preds[t], mu_t, epoch)
            loss_mi += self.model.get_mi_loss(z_preds[t], mu_t)

            if t > 0:
                loss_assign += self._compute_temporal_consistency_loss(s_t, mu_t, z_t_target, s_prev, mu_prev, z_enc_prev)
        ...
        # Normalize losses
        loss_rec, loss_cons, loss_assign, loss_pruning, loss_sparsity, loss_ortho, loss_sep, loss_conn, loss_align, loss_mi = \
            self._normalize_losses(loss_rec, loss_cons, loss_assign, loss_pruning, loss_sparsity, loss_ortho, loss_sep, loss_conn, loss_align, loss_mi, data_list, is_warmup, compute_consistency)

        # Compute final loss with balancing
        loss = self._compute_final_loss(loss_rec, loss_cons, loss_assign, loss_ortho, loss_l2, loss_lvr, loss_align, loss_pruning, loss_sep, loss_conn, loss_sparsity, loss_mi, loss_sym, loss_curv, loss_var, loss_hinge, loss_smooth, compute_consistency, is_stage1)

        # Perform backpropagation
        self._perform_backward_pass(loss, epoch, max_epochs, is_stage1)

        # Update loss tracker
        self._update_loss_tracker(loss, loss_rec, loss_cons, loss_assign, loss_align, loss_mi, loss_sym, loss_var, loss_curv, loss_hinge, loss_smooth)
    ...
    def _handle_sparsity_scheduling(self, epoch):
        """Handle sparsity scheduling logic."""
    ...
    def _determine_training_stage(self, epoch):
        """Determine if we're in stage 1 or warmup."""
        is_stage1 = epoch < 50
        ...
        return is_stage1, is_warmup

    def _configure_model_for_stage(self, is_stage1):
    ...
    def _apply_hard_revival(self, epoch):
        """Apply hard revival of inactive super-nodes."""
        if epoch > 0 and epoch % 50 == 0:
            if hasattr(self.model.encoder.pooling, 'apply_hard_revival'):
                self.model.encoder.pooling.apply_hard_revival()

    def _should_compute_consistency(self, epoch, is_stage1, is_warmup):
        """Determine if consistency loss should be computed."""
        return (epoch >= self.warmup_epochs) and (not is_stage1) and (epoch % self.skip_consistency_freq == 0)

    def _compute_regularization_losses(self, z_preds, dt):
        """Compute L2 and LVR (latent velocity regularization) losses."""
        from common_losses import compute_regularization_losses as common_reg_losses
        return common_reg_losses(z_preds, dt)

    def _compute_hinge_loss(self, z_preds):
        """Compute hinge loss to prevent latent variable shrinkage."""
        from common_losses import compute_hinge_loss as common_hinge_loss
        return common_hinge_loss(z_preds)

    def _compute_assignment_loss(self, losses_t, entropy_weight):
        """Compute assignment loss from individual components."""
        from common_losses import compute_assignment_loss as common_assignment_loss
        return common_assignment_loss(losses_t, entropy_weight)

    def _compute_alignment_loss(self, z_pred_t, mu_t_norm, epoch):
        """Compute alignment loss between predictions and ground truth."""
        ...
        return 2.0 * align_weight * self.criterion(z_pred_t[:, :, :d_align] * torch.exp(self.log_align_scale), mu_t_norm[:, :, :d_align])

    def _compute_temporal_consistency_loss(self, s_t, mu_t, z_t_target, s_prev, mu_prev, z_enc_prev):
        """Compute temporal consistency loss."""
        from common_losses import compute_temporal_consistency_loss as common_temporal_loss
        return common_temporal_loss(self.criterion, s_t, mu_t, z_t_target, s_prev, mu_prev, z_enc_prev)

    def _normalize_losses(self, loss_rec, loss_cons, loss_assign, loss_pruning, loss_sparsity, loss_ortho, loss_sep, loss_conn, loss_align, loss_mi, data_list, is_warmup, compute_consistency):
        """Normalize all computed losses."""
        n_steps = (len(data_list) // 2)
        loss_rec /= n_steps
        loss_cons /= (n_steps - 1) if (not is_warmup and compute_consistency) else 1
        for l in [loss_assign, loss_pruning, loss_sparsity, loss_ortho, loss_sep, loss_conn]: 
            l /= n_steps
        loss_align /= (n_steps * self.model.encoder.n_super_nodes)
        loss_mi /= (n_steps * self.model.encoder.n_super_nodes)
        return loss_rec, loss_cons, loss_assign, loss_pruning, loss_sparsity, loss_ortho, loss_sep, loss_conn, loss_align, loss_mi

    def _compute_final_loss(self, loss_rec, loss_cons, loss_assign, loss_ortho, loss_l2, loss_lvr, loss_align, loss_pruning, loss_sep, loss_conn, loss_sparsity, loss_mi, loss_sym, loss_curv, loss_var, loss_hinge, loss_smooth, compute_consistency, is_stage1):
        """Compute the final combined loss with proper balancing."""
        ...
        if self.enhanced_balancer and not is_stage1:
            ...
            for k in weights:
                if k in balanced and raw_losses[k].item() != 0: 
                    weights[k] = balanced[k] / raw_losses[k]
        ...
        if compute_consistency: 
            loss += (weights['cons'] * torch.clamp(loss_cons.to(torch.float32), 0, 100) + lvars[1])
    ...
    def _perform_backward_pass(self, loss, epoch, max_epochs, is_stage1):
        """Perform the backward pass and parameter updates."""
        if self.enable_gradient_accumulation:
    ...
    def _update_loss_tracker(self, loss, loss_rec, loss_cons, loss_assign, loss_align, loss_mi, loss_sym, loss_var, loss_curv, loss_hinge, loss_smooth):
        """Update the loss tracker with current values."""
        lvars = torch.clamp(self.model.log_vars, min=-6.0, max=5.0)
