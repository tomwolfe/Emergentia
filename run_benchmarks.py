...
def run_trial(mode, potential, noise_std, trial_idx, dim=2):
    ...
    pipeline = DiscoveryPipeline(
        ...
        enable_llm_priors=True  # Enable LLM priors as required by mission contract
    )
    ...
    try:
        ...
        report_path = (
            f"results/report_{mode}_dim{dim}_noise{noise_std}_trial{trial_idx}.txt"
        )
        with open(report_path, "w") as f:
            ...
            f.write(f"LLM Seeds Used: {result.get('llm_priors_enabled', False)}\n")
...
def main():
    ...
    potentials = {
        ...
        "mixed": CompositePotential(
            [HarmonicPotential(k=10.0, r0=1.0), GravityPotential(G=1.0)]
        ),
    }
    ...
    print("=" * 80)
    print(
        df[["mode", "dim", "noise_std", "success", "r2", "mse", "formula"]].to_string(
            index=False
        )
    )
    print("=" * 80)
    ...
    summary_df = (
        df.groupby(["mode", "noise_std"])
        .agg(
            {
                "success": lambda x: (x.sum() / len(x))
                * 100,  # Success rate percentage
                "r2": "mean",
                "mse": "mean",
            }
        )
